                ...
                <table class="table mt-3">
                    ...
                    @foreach($contacts as $contact)
                        <tr>
                            <td>{{$contact->name}}</td>
                            <td>
                                <a class="btn btn-primary" href="{{route('contact.show-more',$contact)}}">Show more</a>
                                <form action="{{ route('contact.destroy', $contact) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')
                                    <input type="hidden" name="currentPage" value="{{ $contacts->currentPage()}}">
                                    <input type="hidden" name="total" value="{{ $contacts->total()}}">
                                    <input type="hidden" name="perPage" value="{{ $contacts->perPage()}}">
            ...
            <div class="modal-content">
                ...
                <form action="{{route('contact.store')}}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        <input type="hidden" class="form-control" value="{{\Illuminate\Support\Facades\Auth::id()}}"
                               name="user_id">
